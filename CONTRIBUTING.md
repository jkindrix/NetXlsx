...
1. **Read [`docs/design.md`](docs/design.md).** It's the long doc but it's the load-bearing one — 52 foundational decisions plus the growing I-NN implementation series, all numbered with rationale (no exact tally quoted: hand-maintained counts only drift). New public API surface should be discussable against an existing decision (or motivate a new numbered one).
...
- **Tests required for behavior changes.** The repo runs five suites per TFM (unit, golden-file, source-generator, fuzz, public-API snapshot) — `bash build/build.sh test` for the live tally; no count is quoted here because hand-maintained counts drift. If your change affects behavior, the test count should change too.
...
The project targets `net8.0` and `net10.0` (both LTS). **The .NET 10 SDK is required** to build the repository — `global.json` pins it with `rollForward: latestFeature` (any `10.x` version is fine). The .NET 8 runtime is also needed for the test matrix.
