...
> **Pinned to NPOI 2.7.3 — a deliberately frozen engine.** NetXlsx pins NPOI at its last Apache-2.0 release (2.7.3); NPOI 2.8+ moves to the OSMF EULA, which would break "MIT all the way down". The trade-off: **upstream NPOI fixes — including security patches — do not flow in automatically**, since adopting them would mean crossing that license boundary or migrating engines ([v2 OOXML planning](docs/v2-ooxml-planning.md)). NetXlsx triages OOXML-parsing vulnerabilities as in-scope (see [SECURITY.md](SECURITY.md)) and mitigates in-tree where it can, but cannot rely on a stream of upstream patches. If you parse untrusted spreadsheets at scale, weigh this posture deliberately.
