...
public class ConcurrencyContractTests
{
    ...
    {
        using var wb = Workbook.CreateOoxml(new WorkbookOptions { StrictConcurrencyDetection = true });
        wb.AddSheet("A");
        wb.AddSheet("B");
        wb.AddSheet("C");
    ...
    public async Task Default_Mode_Surfaces_Concurrent_Mutation_As_InvalidOperation()
    {
        ...
        using var wb = Workbook.CreateOoxml();   // default: StrictConcurrencyDetection = false
        var saw = 0;
