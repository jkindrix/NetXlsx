// I-82 engine swap — OpenOoxml malformed-input gate (pre-cutover parity slice).
//
...
public class OpenMalformedGateTests
{
    ...
    public void Empty_Stream_Throws_MalformedFileException()
    {
        ...
        AssertOpenOoxmlRejects(Array.Empty<byte>());
    }
    ...
    public void Empty_File_Throws_MalformedFileException()
    {
        ...
        try
        {
            ((Action)(() => Workbook.OpenOoxml(path)))
    ...
    public void Empty_Zip_Throws_MalformedFileException()
    {
        ...
        AssertOpenOoxmlRejects(ms.ToArray());
    }
    ...
    public void Valid_Workbook_Still_Opens_Through_The_Gate()
    {
        ...
        byte[] valid = BuildValidWorkbookBytes();
        using var wb = Workbook.OpenOoxml(new MemoryStream(valid), leaveOpen: false);
    ...
    private static void AssertOpenOoxmlRejects(byte[] data)
    {
        ((Action)(() =>
        {
            using var wb = Workbook.OpenOoxml(new MemoryStream(data), leaveOpen: false);
        })).Should().Throw<MalformedFileException>();
    }
