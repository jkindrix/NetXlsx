...
public class OrphanPartPreservationTests
{
    ...
    public void Orphan_Parts_Survive_Open_Modify_Save_BitIdentical()
    {
        ...
        byte[] saved;
        using (var wb = Workbook.Open(new MemoryStream(fixture), leaveOpen: false))
        {
        ...
        using var reopened = Workbook.Open(new MemoryStream(saved), leaveOpen: false);
    ...
    public void Workbook_Without_Orphans_Is_Unaffected()
    {
        ...
        using var reopened = Workbook.Open(new MemoryStream(ms.ToArray()), leaveOpen: false);
