...
// Open XML SDK engine (Workbook.CreateStreaming): the IStreaming* type
...
public class StreamingEngineTests
{
    ...
    {
        using var sw = Workbook.CreateStreaming(new StreamingOptions { MaxCellTextLength = 8 });
        var row = sw.AddSheet("S").AppendRow();
    ...
    public void Save_To_Stream_Honors_LeaveOpen()
    {
        ...
        kept.Position = 0;
        using var wb = Workbook.Open(kept);
    ...
    public void Windowed_Write_Of_Many_Rows_Persists_All_Of_Them()
    {
        ...
        try
        {
            using (var sw = Workbook.CreateStreaming(new StreamingOptions { RowAccessWindowSize = 16 }))
            {
                var sheet = sw.AddSheet("S");
